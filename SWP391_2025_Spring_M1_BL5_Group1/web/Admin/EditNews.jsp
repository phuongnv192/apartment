...
<html lang="en">
    ...
    <body class="sb-nav-fixed">
                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                ...
                                <form action="${pageContext.request.contextPath}/UpdateNewsController" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
                                    <input type="hidden" name="id" value="${news.newId}" />
                                    <div class="mb-3">
                                        <label for="newTitle" class="form-label">Title</label>
                                        <input type="text" class="form-control" id="newTitle" name="newTitle" value="${news.newTitle}" maxlength="100" required>
                                        <span id="titleError" class="error-message"></span>
                                    </div>
                                    <div class="mb-3">
                                        <label for="description" class="form-label">Description</label>
                                        <textarea class="form-control" id="description" name="description" rows="5" maxlength="500" required>${news.description}</textarea>
                                        <span id="descriptionError" class="error-message"></span>
                                    </div>
                                    <div class="mb-3">
                                        <label for="img" class="form-label">Upload Image</label>
                                        <input id="fileInput" type="file" class="form-control" name="img" accept="image/jpeg, image/png">
                                        <span id="fileError" class="error-message"></span>
                                        <c:if test="${not empty news.img}">
                                            <img src="data:image/jpeg;base64,${news.img}" alt="${news.newTitle}" class="img-fluid mt-2" style="max-width: 200px;" />
                                        </c:if>
                                    </div>
                                    <div class="mb-3">
                                        <label for="createAt" class="form-label">Creation Date</label>
                                        <input type="date" class="form-control" id="createAt" name="createAt" value="${news.createAt}" readonly>
                                    </div>
        ...
        <script>
            ...
            if (!createAtInput.value) {
                const today = new Date().toISOString().split('T')[0];
                createAtInput.value = today;
            }
